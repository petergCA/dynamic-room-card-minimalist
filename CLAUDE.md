...
Dynamic Room Card Minimalist is a custom Home Assistant Lovelace card built with Lit (Web Components) and TypeScript. It displays room information in a minimalist style with a room name, icon, optional secondary/tertiary info, and up to 8 entity/template state indicators. Indicators support optional names, a two-column layout, and the card height grows automatically with content.

The card is installed via a custom HA integration in `custom_components/dynamic_room_card_minimalist/` which serves the built JS file over HTTP.
...
npm run deploy           # Production build + copy JS into custom_components/
...
1. `npm run deploy` — builds and copies the JS into `custom_components/dynamic_room_card_minimalist/`
2. Copy the entire `custom_components/dynamic_room_card_minimalist/` folder to your HA config
3. Add `dynamic_room_card_minimalist:` to `configuration.yaml` and restart HA
4. Register resource: `/dynamic_room_card_minimalist/dynamic-room-card-minimalist.js` (JavaScript Module)
...
```
custom_components/dynamic_room_card_minimalist/
├── __init__.py                             # HA integration — registers static HTTP path
├── manifest.json                           # HA integration manifest
└── dynamic-room-card-minimalist.js         # Built JS (generated by `npm run deploy`)
```
...
- Card height is dynamic (`min-height: 236px`), grows with entity count
- `entity_columns` (1 or 2) controls grid layout of entity indicators
...
**Entity Names**: Optional `name` field on each entity renders a label beside the icon.

**Two-Column Layout**: `entity_columns: 2` on the card config switches the entity grid to two columns.

**Dynamic Height**: `:host` uses `min-height` instead of a fixed height; `getCardSize()` calculates grid units from entity count and columns.
